...
axis = ROOT.TH2F('axis', '',1,0,2,7,0,7)
...
txt_dict = {
  'r_ZH': '#splitline{ZH(bb)}{#mu=%.1f#pm%.2f}'%(js['r_ZH']['val'],js['r_ZH']['ErrHi']),
  'r_WH': '#splitline{WH(bb)}{#mu=%.1f#pm%.2f}'%(js['r_WH']['val'],js['r_WH']['ErrHi']),
  'r_zerolep': '#splitline{0 lept.}{#mu=%.1f#pm%.2f}'%(js['r_zerolep']['val'],js['r_zerolep']['ErrHi']),
  'r_onelep': '#splitline{1 lept.}{#mu=%.1f#pm%.2f}'%(js['r_onelep']['val'],js['r_onelep']['ErrHi']),
  'r_twolep': '#splitline{2 lept.}{#mu=%.1f#pm%.2f}'%(js['r_twolep']['val'],js['r_twolep']['ErrHi'])
}
...
plot.DrawTitle(pads[0],'Scenario S1 3000 fb^{-1} (13 TeV)',3)
